...
namespace ExcellyGenLMS.Application.Services.Learner
{
    public class LearnerNotificationService : ILearnerNotificationService
    {
        ...
        public LearnerNotificationService(ILearnerNotificationRepository notificationRepository, ILogger<LearnerNotificationService> logger) { _notificationRepository = notificationRepository; _logger = logger; }
        public async Task<LearnerNotificationDto> CreateNotificationAsync(CreateLearnerNotificationDto createDto) { _logger.LogInformation($"Creating notification for user {createDto.UserId}"); var notification = new LearnerNotification { UserId = createDto.UserId, Title = createDto.Title, Message = createDto.Message, Type = createDto.Type, ProjectId = createDto.ProjectId, ProjectName = createDto.ProjectName, AssignerName = createDto.AssignerName, Role = createDto.Role, WorkloadPercentage = createDto.WorkloadPercentage, IsRead = false, CreatedAt = DateTime.UtcNow }; var createdNotification = await _notificationRepository.CreateAsync(notification); return MapToDto(createdNotification); }
        public async Task<LearnerNotificationSummaryDto> GetUserNotificationSummaryAsync(string userId) { var unreadCount = await _notificationRepository.GetUnreadCountByUserIdAsync(userId); var totalCount = await _notificationRepository.GetTotalCountByUserIdAsync(userId); var recentNotifications = await _notificationRepository.GetRecentByUserIdAsync(userId, 5); return new LearnerNotificationSummaryDto { TotalCount = totalCount, UnreadCount = unreadCount, RecentNotifications = recentNotifications.Select(MapToDto).ToList() }; }
        public async Task<IEnumerable<LearnerNotificationDto>> GetUserNotificationsAsync(string userId, int page = 1, int pageSize = 20) { var notifications = await _notificationRepository.GetByUserIdAsync(userId, page, pageSize); return notifications.Select(MapToDto); }
        public async Task<int> GetUnreadNotificationCountAsync(string userId) { return await _notificationRepository.GetUnreadCountByUserIdAsync(userId); }
        public async Task<bool> MarkNotificationAsReadAsync(int notificationId, string userId) { return await _notificationRepository.MarkAsReadAsync(notificationId, userId); }
...
public async Task CreateProjectUpdateNotificationAsync(string employeeId, string projectId, string projectName, string updateType, string? role, int? workloadPercentage, string assignerName)
{
    ...
    _logger.LogInformation($"Created project update notification for employee {employeeId} on project {projectId}");
        ...
        private static LearnerNotificationDto MapToDto(LearnerNotification notification)
        {
            ...
            var createdAtUtc = DateTime.SpecifyKind(notification.CreatedAt, DateTimeKind.Utc);
            ...
            return new LearnerNotificationDto
            {
                ...
                // formats the DateTime into a universal ISO 8601 string.
